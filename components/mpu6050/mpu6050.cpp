...
MPU6050::MPU6050(int8_t addr, bool run_update_thread) {
	...
	esp32_i2c_write_word(MPU6050_addr, 0x6b, 0b00000000); //Take MPU6050 out of sleep mode - see Register Map

	esp32_i2c_write_word(MPU6050_addr, 0x1a, 0b00000011); //Set DLPF (low pass filter) to 44Hz (so no noise above 44Hz will pass through)

	esp32_i2c_write_word(MPU6050_addr, 0x19, 0b00000100); //Set sample rate divider (to 200Hz) - see Register Map

	esp32_i2c_write_word(MPU6050_addr, 0x1b, GYRO_CONFIG); //Configure gyroscope settings - see Register Map (see MPU6050.h for the GYRO_CONFIG parameter)
...
void MPU6050::getGyroRaw(float *roll, float *pitch, float *yaw) {
	...
	esp32_i2c_read_byte(MPU6050_addr, 0x43, &data1); esp32_i2c_read_byte(MPU6050_addr, 0x44, &data2); //Read X registers
	int16_t X = data1 << 8 | data2;

	esp32_i2c_read_byte(MPU6050_addr, 0x45, &data1); esp32_i2c_read_byte(MPU6050_addr, 0x46, &data2); //Read Y registers
	int16_t Y = data1 << 8 | data2;
	esp32_i2c_read_byte(MPU6050_addr, 0x47, &data1); esp32_i2c_read_byte(MPU6050_addr, 0x48, &data2); //Read Z registers
	int16_t Z = data1 << 8 | data2;
...
void MPU6050::getAccelRaw(float *x, float *y, float *z) {
	uint8_t data1; uint8_t data2;
	esp32_i2c_read_byte(MPU6050_addr, 0x3b, &data1); esp32_i2c_read_byte(MPU6050_addr, 0x3c, &data2); //Read X registers
	int16_t X = data1 << 8 | data2;
	//printf("data1 = %04X\n", data1); USE TO CHECK THAT DATA IS CORRECTLY MANAGED
    //printf("data2 = %04X\n", data2);
    //printf("X = %04X\n", X);
	esp32_i2c_read_byte(MPU6050_addr, 0x3d, &data1); esp32_i2c_read_byte(MPU6050_addr, 0x3e, &data2); //Read Y registers
	int16_t Y = data1 << 8 | data2;
	esp32_i2c_read_byte(MPU6050_addr, 0x3f, &data1); esp32_i2c_read_byte(MPU6050_addr, 0x40, &data2); //Read Z registers
	int16_t Z = data1 << 8 | data2;
